...
//		sSearch: "검색:",
	...
	initComplete : function(oSettings){
		...
		// tooltip 세팅
		$(api.table().body()).find("td").each(function(idx, elem){
			$(elem).attr("title", $(elem)[0].innerText);
		});
	}
});
...
fn_compareTwoDate = function($sObj, $eObj) {
	...
	/**
 	* 고정게시물 순서 변경후 테이블 리로드 역할
 	*
	*/
	function tableAddEvent() {
		// 테이블 순서 변경시 이벤트 발생
		tableObjectFixed.on('row-reorder', function (event, diff, edit ) {
			var _key, _rawOrderNo, _chnOrderNo;
			for ( var i=0, ien=diff.length ; i<ien ; i++ ) {
				if(diff[i].oldData == edit.triggerRow.data().bomKeyId) {
					_key = diff[i].oldData;
					var _listCnt = tableObjectFixed.page.info().recordsTotal;
					_rawOrderNo = _listCnt - diff[i].oldPosition;
					_chnOrderNo = _listCnt - diff[i].newPosition;
					fnChangeOrder(_key, _rawOrderNo, _chnOrderNo);
				}
			}
			callGrid();
		});
