...
/**
***************************************
@ function : 공통 코드 목록 조회 (Promise 버전)
@ comment  :
@ parameter : upCdId - 상위 코드
@ history  : 2022-11-22 (최초작성)
****************************************
*/
function fnGetComnCdList(upCdId) {
	if(!upCdId){
		return Promise.reject("There is no upCdId.");
	}

	return new Promise( (resolve, reject) => {
		jQuery.ajax({
			type: "POST",
			url: "/cmm/fms/getCodeListAjax.do",
			dataType: "json",
			data : { comnCdPcodeId : upCdId },
		}).done(r => resolve(r.beanlist)).fail(reject);
	});
}

/**
***************************************
@ function : 콤보박스 옵션 생성 (fetch)
@ comment  :
@ parameter :	url - 검색 옵션 목록 조회 uri
				responseParam - return 받는 리스트의 파라메터 명
				comboId - 옵션 append 시킬 콤보박스 아이디
				valueCol - 옵션의 value로 설정할 컬럼명
				viewCol - 옵션의 내용으로 설정할 컬럼명
				presetText - value가 없는 검색 옵션의 최상단 기본 옵션
@ history  : 2022-11-25 (최초작성)
****************************************
*/
function fnSetCdComboFetch(uri, responseParam, comboId, valueCol, viewCol, presetText) {
	fetch(uri, {
		method: 'POST',
	})
	...
	.then(data => {
		$(`#${comboId}`)
			.children().remove().end()
			.append(`<option value="">${presetText === undefined ? '전체' : presetText}</option>`)
			.append(`
				${(	() => {
						let option = '';
						for( options of data[responseParam] ){
							option += `<option value="${options[valueCol]}">${options[viewCol]}</option>`
						}
						return option;
					}
				)()}
 ...
 * @param stdNo = 학생 학번
 */
function fnOpenRentListPopup(searchKeyword, contentsParam, rentState, useState){
	$('#POPUP_OPEN_BTN').trigger('click');

	var popupDivId = "#layerPopup";
	var popupUrl = basePath + "/"+ contentsParam +"/r/m/getMngInfoPopupList.do";
	var popupParams = {
	};
 ...
 * @param stdNo = 학생 학번
 */
function fnOpenScheduleListPopup(parameter, contentsParam){
	$('#POPUP_OPEN_BTN').trigger('click');

	var popupDivId = "#layerPopup";
	var popupUrl = basePath + "/"+ contentsParam +"/r/m/getScheduleList.do";
	var popupParams = parameter;
	loadPopup (popupDivId, popupUrl, popupParams);
}
 ...
 * @param korNm = 학생 이름
 */
function fnOpenWinStdInfo(stdNo, korNm, type){
	
	korNm = encodeURI(encodeURIComponent(korNm));
	var _url = "/cmm/fms/getStdHisPopUp.do?stdNo="+ stdNo + "&stdNm=" + korNm + "&type=" + type;
	window.open(_url, "selectPrintForm" ,"toolbar=no, directories=no, scrollbars=yes, resizable=no, status=no, menubar=no, width=1080, height=830, right=10 top=10");
	}
...
****************************************
**/
function fnGoLoginPopUpOld(url) {
...
//첨부파일 다운로드
fn_fileDownload_zip = function(fimReferKeyId, fimFileCategory, fimSectionName, zipName = `files`) {
	if( isEmpty(fimReferKeyId) ){
		alert("첨부파일이 없습니다.");
		return false;
	}
	if( !isEmpty(fimReferKeyId) && !isEmpty(fimFileCategory) && !isEmpty(fimSectionName) ){
		location.href = basePath + "/cmm/fms/fileDownloadZip.do?fimReferKeyId=" + fimReferKeyId
								 + "&fimFileCategory=" + fimFileCategory + "&fimSectionName=" + fimSectionName + "&zipName=" + encodeURI( encodeURIComponent(zipName) );
	}else{
		alert("현재 서비스가 원활하지 않습니다.\n잠시후 다시 이용해 주십시오.");
		console.log("fileCategory or sectionName is undefined");
	}
};
 ...
 * @param selCol					= column use YN (cps_code_mng 테이블 usedef1,usedef2,usedef3.. 활용)
 */
function fnSetComnCdRadio(comnCdPcodeId, viewId, chkdValue, valueName, userGubun, useAsync, validation, events, cdCol, selCol, allValue) {
	var _url = basePath+"/cmm/fms/getCodeListAjax.do";
	...
	if(!isNullObject(allValue)) {
		var allRadioBtn = ` <p class="ad_input_row">
						 		<input name="${valueName}" class="radio" id="${valueName}All" type="radio" value="" title="${allValue}">
						 		<label for="${valueName}All" class="radio_label">${allValue}</label>
						    </p> `;
		targetView.append(allRadioBtn);
	}
		...
		success: function(r) {
			if (r.beanlist.length > 0) {
				for(var idx=0; idx<r.beanlist.length; idx++) {
					...
					if( typeof selCol != "undefined"  && !isEmpty(selCol)){
						if( obj[selCol] == null ){
							targetView.children("p").eq( targetView.children("p").length - 1 ).remove();
						}
					}
					if(!isNullObject(allValue)) {
						 $("#"+ valueName +"All").prop("checked", true);
					}
				}
			}
		},
...
//로그인 화면
function fnGoLoginPopUp(targetUrl, inflowDiv) {

	var popupDivId = "#layerPopup";
	var popupUrl = "/login/a/n/goOpenLoginPopUp.do";
	if( inflowDiv === "PROF" ){
		popupUrl = "/login/a/n/goProfLoginPopUp.do";
	}
	var popupParams = {
						targetUrl
					  };
...
@ function : 로그인 검사
@ comment  :
@ history  : 2022-11-23 (최초작성)
****************************************
*/
function needLoginCheck(){

	// nocnt_box 적용
//	if( !sessionStorage.getItem("sessionUserKeyId") ){
	if( !sessionStorage.getItem("sessionUserId") ){

		// <div>
		$(".needLoginDiv").removeAttr("id");
		$(".needLoginDiv").empty();
		$(".needLoginDiv").append(`
			<div class="nocnt_box">
		...
		// <a> or <button>
		$(".needLoginButton").attr("onclick", "fnGoLoginPopUp();");
 ...
 * @param stfTagId = 값이 들어갈 태그의 아이디
 * comments :
 * 상담사 고유 번호 : '${stfTagId}'+"No"
 * 상담사 이름 : '${stfTagId}'+"Nm"
 * 상담사 전화번호 : '${stfTagId}'+"Phon"
 * 상담사 소속 : '${stfTagId}'+"Dept"
 *
 * @param searchCondition = 검색 구분
 * comments :
 *  이름(addKorNm), 부서(addSustNm)
 ...
 *  검색 후 조회 : N
 */
function cmmnCnslrListPopUp( searchKeyword, stfTagId, searchFlag ) {
	$('#POPUP_OPEN_BTN').trigger('click');

	var popupDivId = "#layerPopup";
	var popupUrl = basePath+"/cmm/fms/getCnslrStaffListPopUp.do";
	var popupParams = {
		// search params
		searchKeyword : searchKeyword
		, searchCondition : 'addKorNm'
 ...
 *  검색 후 조회 : N
 */
function fnOpenCertiListPopUp(searchKeyword, mngType, searchFlag) {
	$('#POPUP_OPEN_BTN').trigger('click');

	var popupDivId = "#layerPopup";
	var popupUrl = basePath+"/cmm/fms/getCertiInfoPopUp.do";
	var popupParams = {
		// search params
		searchKeyword : searchKeyword
		, searchFlag : searchFlag
...
function makeToDateTime(_date, _type) {
	if (!isEmpty(_date)) {
		...
		switch (_type) {
			case "dateTimeSeconds":
				...
				break;
			case "dateTimehours":
				formattedDateTime = year + "-" + month + "-" + day + " " + hours;
				break;
			case "dateTime":
...
function getDayOfTheWeek(_langType) {
    ...
    var dayName;
    if(_langType == "en") {
    	dayName = enDays[dayOfWeek];
    } else {
    	dayName = korDays[dayOfWeek];
    }
