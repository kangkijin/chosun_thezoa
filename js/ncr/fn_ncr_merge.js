...
/**
  ...
  **/
  function fnNcrProgramCodeMngPopUp(codeType,viewType, pciKeyId){
    ...
    var popupUrl = basePath+"/ncrProgramCodeMng/r/n/getProgramCodeListDetailPopUp.do"; // 팝업 내용을 호출하는 url
    var popupParams = {
        viewType : viewType,
        pciKeyId : pciKeyId,
        codeType : codeType
...
function fnInputValidation() {
	...
	if(nullValidation){
        ...
        if ($('#npiSubject').val() == null || $('#npiSubject').val() == '') {
        ...
        if (isEmpty($("#npiGoalsOverView").val())) {
            alert("프로그램 목표 및 개요 입력해주세요");
            $("#npiGoalsOverView").focus();
            return false;
        }
        
        if (isEmpty($("#npiApplPrecautions").val())) {
            alert("프로그램 주요 내용 및 유의사항 입력해주세요");
            $("#npiApplPrecautions").summernote('focus');
            return false;
        }
	}//nullValidation
	// return false;
  ...
  $.ajax({
    ...
    success : function(res) {
      if (res.stafInfoList.length > 1) {
        ...
        var nmTagId = stfNmTagId.substring(0,stfNmTagId.length-2);
        // 전화번호랑 이메일을 추가적으로 인자로 넘김)
        fnReceiveChkStaffInfo(res.stafInfoList[0].staffNo,res.stafInfoList[0].korNm,nmTagId, res.stafInfoList[0].phone, res.stafInfoList[0].email);
        $("#npiStfHp1").focus();
      }
...
function fnSetUpNcrProgramCode(upCode, selValue) {
    if (isEmpty(selValue)) {
      $("select#npcSbjtCd option").remove();
    }
   var progracmCnt = fnGetNcrProgramSubCode(upCode, "npcSbjtCd", selValue, '중위프로그램을 선택해주세요', false);
}
     ...
     **********************************************************
   **/
function fnGetNcrProgramSubCode(comnCdPcodeId, viewId, selValue, allValue, useAsync) {
    var basePath = sessionStorage.getItem("contextRootPath");
    ...
    jQuery.ajax({
      ...
      success: function(r) {
        if (r.beanlist.length > 0) {
          for(var idx=0; idx<r.beanlist.length; idx++) {
            var obj = r.beanlist[idx];
            if(typeof selValue != "undefined" && selValue == obj.PSC_SUBJ_CD) {
              targetView.append("<option value='"+obj.NPC_SBJT_CD+"' selected>"+obj.NPC_SBJT_NM+"</option>");
            } else {
              targetView.append("<option value='"+obj.NPC_SBJT_CD+"'>"+obj.NPC_SBJT_NM+"</option>");
            }
          }
        }
      },
...
function fnSetProgramCode(porgramCd) {
    ...
    if (isEmpty(porgramCd)) {
        var fistElement = $('tr[name="appliedSubject1"]')
        var lastElement = $('tr[name="target2"]');
        fistElement.hide();
        lastElement.hide();
    ...
    $.ajax({
        url : basePath+"/ncrProgramRegSTF/r/n/getgetTotalTimeOfSubCode.do"
        , type : "post"
        , data : {
            npcSbjtCd : $("#npcSbjtCd").val()
        }
        , success : function(res) {
             if (res) {
                var npcProMainAbility = res.mileageAndCom.NPC_PRO_MAIN_ABILITY_CD;
                var npcMainAbility = res.mileageAndCom.NPC_MAIN_ABILITY_CD;
                var npcLearnAbility1Cd = res.mileageAndCom.NPC_LEARN_ABILITY_1_CD;
                var npcLearnAbility2Cd = res.mileageAndCom.NPC_LEARN_ABILITY_2_CD;
                $("#npiProgramCnt").val(res.totalTime);
                
                if (res.mileageAndCom.NPC_PROGRAM_TYPE == 'NCR_STD') {
                    $('tr[name="appliedSubject1"]').show();
                    $('tr[name="target2"]').hide();
                } else {
                    $('tr[name="appliedSubject1"]').hide();
                    $('tr[name="target2"]').show();
                }
                fnCheckNameOfCode(npcProMainAbility, npcMainAbility, npcLearnAbility1Cd, npcLearnAbility2Cd);
            }
        }
    });
}
...
****************************************
 **/
function fnGoDetailInfo(_keyId){

  var _url =basePath+"/ncrProgramRegSTF/r/n/getProgramDetail.do";
  _url += "?npiKeyId=" + encodeURI(encodeURIComponent(_keyId));
  window.open(_url, "npiDetail", 'height=' + (screen.height - 100) + ',width=' + screen.width + 'fullscreen=yes, scrollbars=yes');
  /*$('#detailForm').attr("action",_url).submit();*/
}

function fnCheckNameOfCode(npcProMainAbility, npcMainAbility, npcLearnAbility1Cd, npcLearnAbility2Cd) {
    var str = "";
    var item = "";
    if (!isEmpty(npcProMainAbility)) {
        item = npcProMainAbility.split(",").map(function(item){
            return $.trim(item);
        });
        var lastElement = item[item.length - 1];
        var getClassDom = document.getElementsByClassName("getproMainAbility");
        for (var i = 0; i <= item.length; i++) {
            for (var j = 0; j < getClassDom.length; j++) {
                if (item[i] == getClassDom[j].value) {
                    if (item[i] != lastElement) {
                        str += getClassDom[j].title + ",";
                    } else {
                        str += getClassDom[j].title + ".";
                    }
                }
        ...
        $("#proMainAbilityCd").html(str);
    }
    
    if (!isEmpty(npcMainAbility)) {
        var getClassDom = document.getElementsByClassName("getMainAbility");
        for (var i = 0; i < getClassDom.length; i++) {
            if (npcMainAbility == getClassDom[i].value) {
                str = getClassDom[i].title;
            }
            if (!isEmpty(npcLearnAbility1Cd) && !isEmpty(npcLearnAbility2Cd)) {
                if (npcLearnAbility1Cd == getClassDom[i].value) {
                    var str1 = getClassDom[i].title;
                }
                
                if (!isEmpty(str1)) {
                    if (npcLearnAbility2Cd== getClassDom[i].value) {
                        str1 += ", " + getClassDom[i].title;
                    }
                }
            }
        }
        $("#mainAbility").html(str);
        $("#secondaryAbility").html(str1);
    }
}